...
def train_with_config(args, opts):
    ...
    # with open("data/action/ntu_actions.txt", 'r') as file:
    #     labels = [label.split(".")[1] for label in file.readlines()]
    ...
    # # ntu120_xsub_val = NTURGBD(data_path=data_path, data_split=args.data_split+'_val', n_frames=args.clip_len, random_move=False, scale_range=args.scale_range_test)
    # video_dir = PROJECT_ROOT / "check_dataset"
    # ac = ActionClassifier()
    # for i in range(120):
    #     label_idx = ntu60_xsub_val.labels[i]
    #     label = labels[label_idx].replace("/", "").replace("\\", "")
    #     fpath1 = video_dir / f'{i // 60}_ax0_{label}.mp4'
    #
    #     if not fpath1.is_file():
    #         points1 = ntu60_xsub_val.motions[i, 0, ...]
    #         print(ac.inference(points1))
    #         render_and_save(points1, str(fpath1), keep_imgs=False, fps=25)
    #
    #     # fpath2 = video_dir / f'{i // 60}_ax1_{label}.mp4'
    #     # if not fpath2.is_file():
    #     #     points2 = ntu60_xsub_val.motions[i, 1, ...]
    #     #     render_and_save(points2, str(fpath2), keep_imgs=False, fps=25)
    #
    # exit(0)
    #
    # ntu60_xsub_train = NTURGBD(data_path=data_path, data_split=args.data_split + '_train',
    #                            n_frames=args.clip_len, random_move=args.random_move,
