...
from lib.data.interpolation import Interpolation
...
def human_tracking(x):
    ...
    if M == 1:
        ...
        diff0 = np.sum(np.linalg.norm(x[0, 1:] - x[0, :-1], axis=-1),
                       axis=-1)  # (T-1, V, C) -> (T-1)
...
class ActionDataset(Dataset):
    def __init__(
        ...
        self.motion_stretch_range = motion_stretch_range
        self.move_start = move_start
        self.target_len = n_frames
        motions = []
        labels = []
        for sample in annotations:
            if check_split and (not sample['frame_dir'] in self.split):
                continue
            # resample_id = resample(ori_len=sample['total_frames'], target_len=n_frames,
            ...
            # motion = np.concatenate((motion_cam[:, resample_id], motion_conf[:, resample_id]),
            #                         axis=-1)
        ...
        self.interpolator = Interpolation() if motion_stretch_range is not None else None
    ...
    def __getitem__(self, idx):
        ...
        if self.motion_stretch_range is not None:
            motion = self.stretch_motion(motion)
        start_pos = self.get_random_start_pos(motion.shape[1]) if self.move_start else 0
        resample_id = self.get_resample_ids(motion.shape[1], start_pos)
        motion = motion[:, resample_id]
        return motion.astype(np.float32), label

    def stretch_motion(self, motion):
        stretch_range = np.random.uniform(*self.motion_stretch_range)
        motion = self.interpolator.stretch_motion_bert(motion, stretch_range)
        return motion

    def get_random_start_pos(self, data_length: int) -> int:
        max_move_distance = self.target_len - data_length
        start_pos = np.random.randint(1, max_move_distance) if max_move_distance > 1 else 0
        return start_pos

    def get_resample_ids(self, data_length: int, start_pos: int) -> list:
        if start_pos > 0 or data_length < self.target_len:
            resample_id = [0] * start_pos + \
                          [i for i in range(data_length)] + \
                          [data_length - 1] * (self.target_len - data_length - start_pos)
        else:
            resample_id = [i for i in range(self.target_len)]
        return resample_id


class NTURGBD(ActionDataset):
    def __init__(self, data_path, data_split, **kwargs):
        super(NTURGBD, self).__init__(data_path, data_split, **kwargs)


class NTURGBD1Shot(ActionDataset):
    def __init__(self, data_path, data_split, n_frames=243, random_move=True, scale_range=[1, 1],
                 check_split=False):
        super(NTURGBD1Shot, self).__init__(data_path, data_split, n_frames, random_move,
                                           scale_range, check_split)
        oneshot_classes = [0, 6, 12, 18, 24, 30, 36, 42, 48, 54, 60, 66, 72, 78, 84, 90, 96, 102,
                           108, 114]
