...
class PCT(BasePose):
    ...
    def forward_test(self, img, joints, img_metas, **kwargs):
        ...
        output = None if self.stage_pct == "tokenizer" else self.backbone(img)
        extra_output = self.extra_backbone(img) if self.image_guide and self.stage_pct == "tokenizer" else None
        ...
        if self.flip_test:
            ...
            features_flipped = None if self.stage_pct == "tokenizer" else self.backbone(img_flipped)
            extra_output_flipped = self.extra_backbone(img_flipped) if self.image_guide and self.stage_pct == "tokenizer" else None
            ...
            p_joints_f, encoding_scores_f = self.keypoint_head(features_flipped, extra_output_flipped, joints_flipped, train=False)
        ...
        for i in range(p_joints.shape[0]):
            p_joints[i] = transform_preds(p_joints[i], c[i], s[i], [img.shape[-1], img.shape[-2]], use_udp=False)
